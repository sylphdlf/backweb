    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 访问记录
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
...
<!--                <el-input v-model="query.name" placeholder="角色" class="handle-input mr10"></el-input>-->
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header" size="mini">
                ...
                <el-table-column label="最后访问时间" >
                    <template slot-scope="scope">
                        <span v-if="scope.row.accessCount === 1">{{scope.row.createTime | dateTime}}</span>
                        <span v-if="scope.row.accessCount > 1">{{scope.row.updateTime | dateTime}}</span>
                    </template>
                </el-table-column>
                <el-table-column label="操作" width="180" align="center">
                    <template slot-scope="scope">
                        <el-button type="text" icon="el-icon-close" class="red" @click="handleEdit(scope.$index, scope.row)">踢下线</el-button>
                        <el-button type="text" icon="el-icon-s-custom" class="red" @click="handleDelete(scope.$index, scope.row)">拉黑</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
export default {
    ...
    data() {
        return {
            urlSearch: "/server/queryAccessPage",
